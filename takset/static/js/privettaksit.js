const TaksitSystemprivet = {
  ...
  elements: {
    ...
    btnPrint: document.getElementById("btnpriv"),
  ...
  Payload_privet: function () {
    const el = this.elements;

    // 1. تجميع كل البيانات في كائن واحد (طرد بريدي)
    return {
      personal_info_privet: {
        full_name: document.getElementById("full_name").value,
        phonenum: document.getElementById("phonenum").value,
        numrecip: document.getElementById("numrecip").value,
        rahn: document.getElementById("rahn").value,
        datreminder: document.getElementById("datreminder").value,
        datepaid: document.getElementById("datepaid").value,
      },
      financial_info_privet: {
  ...
  init: function () {
    ...
    if (el.btnPrint) {
      el.btnPrint.onclick = (e) => {
        ...
        const data = self.Payload_privet(); // جلب البيانات المحدثة
        const csrf = document.querySelector("[name=csrfmiddlewaretoken]").value;

        console.log("إرسال البيانات:", data);

        fetch("/addnew/privettaksit.html/", {
          method: "POST",
          headers: {
          ...
          .then((res) => res.json())
          .then((resData) => alert("رد السيرفر: " + resData.msg))
...
if (dateInput) {
  ...
  console.log("تم تعيين التاريخ بنجاح");
} else {
