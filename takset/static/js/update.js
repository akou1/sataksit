  ...
  init: function () {
    ...
    if (el.btnPrint) {
      el.btnPrint.onclick = (e) => {
        ...
        // هذا السطر يقوم بتقسيم الرابط الحالي واستخراج الرقم المتواجد فيه تلقائياً
        const pathArray = window.location.pathname.split("/");
        // تصفية المصفوفة لحذف الفراغات وجلب الرقم
        const idValue = pathArray.filter((item) => item !== "").pop();

        console.log("الـ ID الذي تم التقاطه من الرابط هو:", idValue);
        console.log(idValue);
        fetch(`/update/${idValue}/`, {
          method: "POST",
          headers: {
          ...
          .then((res) => res.json())
          .then((resData) => {
            // السيرفر يرسل رسالة باسم 'message' وليس 'msg'
            alert("رد السيرفر: " + (resData.message || resData.msg));
